...
export function HelpWidget() {
  ...
  return (
    ...
    >
      {open && (
        <>
          {/* Mobile: centered pop-up modal, independent of the button's dragged position */}
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm sm:hidden">
            <div className="helper-pop-in flex h-[70vh] max-h-[32rem] w-full max-w-sm flex-col overflow-hidden rounded-3xl border border-black/10 bg-white shadow-2xl dark:border-white/10 dark:bg-pulse-charcoal">
              <HelperPanelContents
                messages={messages}
                loading={loading}
                input={input}
          ...
          {/* Desktop: panel anchored just above the button */}
          <div className="helper-pop-in absolute bottom-full right-0 mb-4 hidden h-[28rem] max-h-[70vh] w-80 flex-col overflow-hidden rounded-3xl border border-black/10 bg-white shadow-2xl dark:border-white/10 dark:bg-pulse-charcoal sm:flex">
            <HelperPanelContents
              messages={messages}
              loading={loading}
              input={input}
      ...
      <style>{`
        .helper-pop-in { animation: helperPopIn 0.22s cubic-bezier(0.16, 1, 0.3, 1) both; }
        @keyframes helperPopIn {
  ...
  return (
    <>
      <div className="flex items-center justify-between border-b border-black/5 p-4 dark:border-white/10">
        ...
        <button onClick={onClose} className="text-black/40 hover:text-black dark:text-white/40 dark:hover:text-white">
